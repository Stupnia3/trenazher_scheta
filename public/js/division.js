...
class Train {
    ...
    checkAnswer() {
        const rightAnswer = this.numbers_array[this.count].reduce((a, b) => a / b);
    ...
    generateNumbers() {
        ...
        for (let i = 0; i < this.setting.examples.value; i++) {
            ...
            // Генерируем результат деления
            let result = randomInteger(minValue, maxValue - 1);
            newSequence.push(result);

            for (let j = 1; j < this.setting.action_count.value; j++) {
                let divisor = randomInteger(2, 10); // Делаем так, чтобы делитель был от 2 до 10
                result *= divisor; // Обратное умножение для генерации числа, которое точно делится
                newSequence.push(divisor);
            }

            newSequence[0] = result; // Устанавливаем результат деления первым числом
